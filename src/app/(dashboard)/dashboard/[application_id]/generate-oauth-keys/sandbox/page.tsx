...
const TokenGenerationPage = () => {
  ...
  const { id } = useUser();
  const { consumerKey, consumerSecret, fetchOAuthKeys } = useBuyerKeysStore();
        ...
        >
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage>Sandbox Keys</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </motion.div>
                  ...
                  {consumerKey && (
                    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      <Button
                        variant="outline"
                        size="icon"
                        onClick={() => copyToClipboard(consumerKey, "Consumer key")}
                  ...
                  {consumerSecret && (
                    <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                      <Button
                        variant="outline"
                        size="icon"
                        onClick={() => copyToClipboard(consumerSecret, "Consumer secret")}
                        className="hover:bg-primary hover:text-primary-foreground transition-all duration-300"
                      >
                        <Copy className="h-4 w-4" />
                      </Button>
